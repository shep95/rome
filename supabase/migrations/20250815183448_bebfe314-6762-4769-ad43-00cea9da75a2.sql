-- Create a function to check if a user can access a secure file based on conversation participation
...
-- Add policy for conversation participants to access shared secure files
CREATE POLICY "Conversation participants can view shared secure files"
ON storage.objects
FOR SELECT
USING (
  bucket_id = 'secure-files' 
  AND public.user_can_access_file(name)
);